document.addEventListener("DOMContentLoaded", () => {
  ...
  if (buscador) {
    buscador.addEventListener('keypress', function (e) {
      if (e.key === 'Enter') {
        ...
        if (texto.length > 0) {
          window.location.href = 'explorar.php?buscar=' + encodeURIComponent(texto);
        }
      }
    });
  }
  ...
  if (filtroGenero) {
    filtroGenero.addEventListener('change', renderizarExplorar);
  }

  let filtroAutor = document.getElementById('filter-autor');
  if (filtroAutor) {
    filtroAutor.addEventListener('input', renderizarExplorar);
  }

  let filtroValoracion = document.getElementById('filter-valoracion');
  if (filtroValoracion) {
    filtroValoracion.addEventListener('change', renderizarExplorar);
  }
