document.addEventListener("DOMContentLoaded", () => {
    ...
    document.querySelectorAll('.pestanias-tab').forEach(btn => {
    ...
    if (buscador) {
        buscador.addEventListener('keypress', function (e) {
            if (e.key == 'Enter') {
                ...
                if (texto.length > 0) {
                    globalThis.location.href = globalThis.BASE_URL + '/inicio/explorar?buscar=' + encodeURIComponent(texto);
                }
            }
        });
    }
    ...
    if (cajaEstrellas) {
        ...
        estrellas.forEach(boton => {
            boton.addEventListener('click', () => {
                ...
                const elegida = Number.parseInt(boton.dataset.val);
